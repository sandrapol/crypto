...
namespace Crypto
{
    class DESpacito
    {
        ...
        //Circular Right shify table (for Decryption)
        ...
        private int ConvertTextToBits(char[] charrarray, int[] savedarray)
        {
            int j = 0;
            for (int i = 0; i < charrarray.Length; ++i)
            {
                int[] ba = BitArray.ToBits(GetASCII(charrarray[i]), 8);
                j = i * 8;
        ...
        public string Encrypt(string plaintext, string key)
        {
            ...
            //Konwertowanie liter tekstu szyfrowanego na postać binarną
            int st = ConvertTextToBits(ptca, plaintextbin);

            int fst = AppendZeroes(plaintextbin, st);

            //Konwertowanie liter klucza na postać binarną
            ...
            cyphertext = ConvertBitsToText(ciphertextbin, fst);
        ...
        public string Decrypt(string ciphertext, string key)
        {
            ...
            //Konwertowanie liter z tekstu zaszyfrowanego na postać binarną
            int st = ConvertTextToBits(ctca, ciphertextbin);

            int fst = AppendZeroes(ciphertextbin, st);

            //Konwertowanie liter z klucza na postać binarną
            int sk = ConvertTextToBits(kca, keybin);
